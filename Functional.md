...
While you can use any Clojure elements as keys, most commonly keywords are used for this purpose since look-up by keywords is very fast:
...
If a key doesn't appear in a map, all three ways of look-up will return `nil`:
...
So far we've seen how we can use built-in functions, but how do we define our own?
...
For example, consider determining if a string is a palindrome. In a traditional approach one would have a loop in which an index is changing as the string is being traversed that compares the string characters to each other. In a functional approach one would just compare the string to its reverse and return the result:
...
Let's walk through an example to see how the recursion plays out.
Starting with:

```user=> (holds-for-all? odd? [1 3 4 5])```

Every time we enter `holds-for-all?` the parameter `f` will
be bound to the function `odd?` (so at least that part is easy)!

The first time we enter `(= v [1 3 4 5])`. So `(rest v)` is `'(3 4 5)` and
thus NOT empty?. Therefore we will return the logical and of
the function applied to the first element `1` and then recurse
on the rest of the list. Since `(odd? 1)` is `true` the next
argument to the `and` function is evaluated (and we recurse).

The second time we enter `(= v (3 4 5))` and  `(rest v)` is `'(4 5)`
which is NOT empty? so we return the value of the logical and.
Here `(odd? 3)` is `true` so we recurse on the rest of the list.

The third time we enter  `(= v (4 5))` and  `(rest v)` is `'(5)`
which is NOT empty? so we return the value of the logical and.
Here `(odd? 4)` is `false` so the logical and is *short circuited*
and returns false *right away* (since one of the arguments is
false we know the entire expression is false).

As Clojure *pops the stack* and returns to the second time we
entered the `holds-for-all?` we now have the value of calling
the function the third time: `false`. As `(and true false)` value
of the second invocation is `false`.

Finally Clojure *pops the stack* again and control returns to the
first time time we entered the function and we have the value of
the function for the rest of the list which is `false`.
Therefore `(and true false)` returns `false` and that's our final
answer: `odd?` does *not* hold for all elements of `[1 3 4 5]`
because `4` is even!
